...
// Puedes dejar esta función si quieres mantener una copia local de respaldo, 
// o quitarla si todo irá directo a la BD. Se deja por si acaso.
...
// ══════════════════════════════════════════
//  FUNCIÓN PUBLICAR (CON FETCH API)
// ══════════════════════════════════════════
async function publicar() {
  const exp = new Date();
  exp.setDate(exp.getDate() + 60);

  // Formateamos la fecha de expiración a YYYY-MM-DD para compatibilidad SQL
  const fechaCaducidadFormateada = exp.toISOString().split('T')[0];

  // Construimos el payload con los nombres exactos que espera el controlador backend
  const payload = {
    id_mascota:       state.mascota.id,
    recompensa:       state.recompensa,
    comentarios:      state.resumen,
    fecha_expedicion: state.fecha, // Ya viene en formato YYYY-MM-DD del input date
    fecha_caducidad:  fechaCaducidadFormateada,
    calle:            state.ubicacion.calle,
    colonia:          state.ubicacion.colonia,
    alcaldia:         state.ubicacion.alcaldia,
    codigo_postal:    state.ubicacion.codigoPostal
  };

  try {
    // Bloquear el botón mientras se procesa la petición
    const btnSubmit = document.getElementById('btn-p4');
    ...
    if (!respuesta.ok) {
      ...
      throw new Error(errorData.error || 'Ocurrió un error en el servidor al intentar guardar');
    }

    // Respuesta exitosa del backend
    const datosGuardados = await respuesta.json();
    console.log('Reporte guardado exitosamente:', datosGuardados);

    // Pantalla de Éxito
    document.getElementById('exito-desc').innerHTML =
      `El reporte de <strong>${state.mascota.nombre}</strong> fue publicado con éxito. Estará activo por <strong>60 días</strong>.`;
    document.getElementById('ex-ruac').textContent      = state.mascota.ruac;
    document.getElementById('ex-ubi').textContent       = `${state.ubicacion.colonia}, ${state.ubicacion.alcaldia}`;
    document.getElementById('ex-recompensa').textContent= formatMXN(state.recompensa) + ' MXN';
    document.getElementById('ex-expira').textContent    = formatFecha(exp.toISOString());

    mostrarPaso('exito');

  } catch (error) {
    console.error("Error al publicar el reporte:", error);
    alert("Hubo un problema al publicar el reporte: " + error.message);
  } finally {
    // Restaurar el botón en caso de error o éxito
    const btnSubmit = document.getElementById('btn-p4');
